...
class ColorSlider(Tkinter.Scale): # ColorSlider is a subclass of Tkinter.Scale
    ...
    def __init__(self, parent, myLabel, model_intvar, editor, canvas):
        ...
        def slider_changed(new_val):
            ...
            canvas.itemconfig(Rectangles[-1],fill=tk_color_string)
...
#    new_rectangle = canvas.create_rectangle(x-r, y-r, x+r, y+r, outline='#000000')
#    Rectangles.append(new_rectangle) # aggregate a list of the shapes on the canvas
#canvas.itemconfig(Rectangles[4],outline='green')
...
def up(event):
    ...
    Rectangles.append(new_shape) # aggregate the canvas' item
